...
import { Download, Shield, Clock, CheckCircle, ExternalLink, Tv, Calendar, Tag } from "lucide-react"
...
export default function EpisodeDownloadClient({
  series,
  episode,
  adSettings,
}: {
  ...
  const handleDownload = () => {
    if (canDownload && episode.downloadUrl) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <div className="w-32 h-48 flex-shrink-0 rounded-lg overflow-hidden">
            <img
              src={series.posterUrl || "/placeholder.svg?height=192&width=128"}
              alt={series.title}
              className="w-full h-full object-cover"
            />
          </div>
          <div className="flex-1">
            <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">{series.title}</h1>
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 text-[#00FFFF] hover:underline"
            >
        ...
        {adSettings?.showDownloadPageAds && adSettings?.horizontalAdCode && (
          <motion.div className="mb-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
            <div className="bg-[#1A1B23] border border-[#2A2B33] rounded-lg overflow-hidden">
              <iframe
                srcDoc={`<!DOCTYPE html><html><head><style>body { margin: 0; padding: 8px; display: flex; align-items: center; justify-content: center; background: transparent; }</style></head><body>${adSettings.horizontalAdCode}</body></html>`}
