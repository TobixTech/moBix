...
export default function SeriesCard({ series }: SeriesCardProps) {
  ...
  const { trackCardClick } = useAdClickTracker()
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    const shouldShowAd = trackCardClick()

    if (shouldShowAd) {
      e.preventDefault()
      setPendingNavigation(seriesUrl)
  ...
  return (
    <>
      {showInterstitialAd && pendingNavigation && (
        <InterstitialAd
          adCode="<!-- Your vertical ad code here -->"
          onClose={() => {
      ...
      <Link href={seriesUrl} prefetch={false} onClick={handleCardClick}>
        <motion.div
